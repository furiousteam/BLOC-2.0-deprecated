...
std::string Simple_WalletRPC::get_commands_str() {
  std::stringstream ss;
  ss << "Commands: " << ENDL;
...
bool Simple_WalletRPC::help(const std::vector<std::string> &args/* = std::vector<std::string>()*/) {
  success_msg_writer() << get_commands_str();
  return true;
}

bool Simple_WalletRPC::exit(const std::vector<std::string> &args) {
  m_consoleHandler.requestStop();
  return true;
}

Simple_WalletRPC::Simple_WalletRPC(System::Dispatcher& dispatcher, const CryptoNote::Currency& currency, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("start_mining", boost::bind(&Simple_WalletRPC::start_mining, this, _1), "start_mining [<number_of_threads>] - Start mining in daemon");
  m_consoleHandler.setHandler("stop_mining", boost::bind(&Simple_WalletRPC::stop_mining, this, _1), "Stop mining in daemon");
  //m_consoleHandler.setHandler("refresh", boost::bind(&Simple_WalletRPC::refresh, this, _1), "Resynchronize transactions and balance");
  m_consoleHandler.setHandler("balance", boost::bind(&Simple_WalletRPC::show_balance, this, _1), "Show current wallet balance");
  m_consoleHandler.setHandler("incoming_transfers", boost::bind(&Simple_WalletRPC::show_incoming_transfers, this, _1), "Show incoming transfers");
  m_consoleHandler.setHandler("list_transfers", boost::bind(&Simple_WalletRPC::listTransfers, this, _1), "Show all known transfers");
  m_consoleHandler.setHandler("payments", boost::bind(&Simple_WalletRPC::show_payments, this, _1), "payments <payment_id_1> [<payment_id_2> ... <payment_id_N>] - Show payments <payment_id_1>, ... <payment_id_N>");
  m_consoleHandler.setHandler("bc_height", boost::bind(&Simple_WalletRPC::show_blockchain_height, this, _1), "Show blockchain height");
  m_consoleHandler.setHandler("transfer", boost::bind(&Simple_WalletRPC::transfer, this, _1),
  ...
  m_consoleHandler.setHandler("set_log", boost::bind(&Simple_WalletRPC::set_log, this, _1), "set_log <level> - Change current log level, <level> is a number 0-4");
  m_consoleHandler.setHandler("address", boost::bind(&Simple_WalletRPC::print_address, this, _1), "Show current wallet public address");
  m_consoleHandler.setHandler("keys", boost::bind(&Simple_WalletRPC::print_keys, this, _1), "Show wallet private keys");
  m_consoleHandler.setHandler("save", boost::bind(&Simple_WalletRPC::save, this, _1), "Save wallet synchronized data");
  m_consoleHandler.setHandler("reset", boost::bind(&Simple_WalletRPC::reset, this, _1), "Discard cache data and start synchronizing from the start");
  m_consoleHandler.setHandler("help", boost::bind(&Simple_WalletRPC::help, this, _1), "Show this help");
  m_consoleHandler.setHandler("exit", boost::bind(&Simple_WalletRPC::exit, this, _1), "Close wallet");
}
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::set_log(const std::vector<std::string> &args) {
  if (args.size() != 1) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::init(const boost::program_options::variables_map& vm) {
  ...
  Tools::PasswordContainerRPC pwd_container;
...
//----------------------------------------------------------------------------------------------------
void Simple_WalletRPC::handle_command_line(const boost::program_options::variables_map& vm) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::new_wallet(const std::string &wallet_file, const std::string& password) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::close_wallet()
{
  try {
    CryptoNote::WalletHelper::storeWallet(*m_wallet, m_wallet_file);
  } catch (const std::exception& e) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::save(const std::vector<std::string> &args)
{
  try {
    CryptoNote::WalletHelper::storeWallet(*m_wallet, m_wallet_file);
...
bool Simple_WalletRPC::reset(const std::vector<std::string> &args) {
  {
...
bool Simple_WalletRPC::start_mining(const std::vector<std::string>& args) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::stop_mining(const std::vector<std::string>& args)
{
...
//----------------------------------------------------------------------------------------------------
void Simple_WalletRPC::initCompleted(std::error_code result) {
  if (m_initResultPromise.get() != nullptr) {
    m_initResultPromise->set_value(result);
  }
}
//----------------------------------------------------------------------------------------------------
void Simple_WalletRPC::connectionStatusUpdated(bool connected) {
  if (connected) {
...
//----------------------------------------------------------------------------------------------------
void Simple_WalletRPC::externalTransactionCreated(CryptoNote::TransactionId transactionId)  {
...
//----------------------------------------------------------------------------------------------------
void Simple_WalletRPC::synchronizationCompleted(std::error_code result) {
  std::unique_lock<std::mutex> lock(m_walletSynchronizedMutex);
  m_walletSynchronized = true;
  m_walletSynchronizedCV.notify_one();
}

void Simple_WalletRPC::synchronizationProgressUpdated(uint32_t current, uint32_t total) {
  std::unique_lock<std::mutex> lock(m_walletSynchronizedMutex);
...
bool Simple_WalletRPC::show_balance(const std::vector<std::string>& args/* = std::vector<std::string>()*/) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::show_incoming_transfers(const std::vector<std::string>& args) {
...
bool Simple_WalletRPC::listTransfers(const std::vector<std::string>& args) {
...
bool Simple_WalletRPC::show_payments(const std::vector<std::string> &args) {
  if (args.empty()) {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::show_blockchain_height(const std::vector<std::string>& args) {
  try {
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::transfer(const std::vector<std::string> &args) {
  try {
...
//----------------------------------------------------------------------------------------------------
void Simple_WalletRPC::stop() {
  m_consoleHandler.requestStop();
}
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::print_address(const std::vector<std::string> &args/* = std::vector<std::string>()*/) {
  success_msg_writer() << m_wallet->getAddress();
  return true;
}
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::print_keys(const std::vector<std::string> &args/* = std::vector<std::string>()*/) {
	AccountKeys keys;
	m_wallet->getAccountKeys(keys);	
...
//----------------------------------------------------------------------------------------------------
bool Simple_WalletRPC::process_command(const std::vector<std::string> &args) {
  return m_consoleHandler.runCommand(args);
}

void Simple_WalletRPC::printConnectionError() const {
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      ...
      CryptoNote::Simple_WalletRPC tmp_wallet(dispatcher, tmp_currency, logManager);
  ...
  if (command_line::has_arg(vm, Tools::wallet_rpc_server::arg_rpc_bind_port)) {
	...
	*/
    ...
    CryptoNote::Simple_WalletRPC wal(dispatcher, currency, logManager);
