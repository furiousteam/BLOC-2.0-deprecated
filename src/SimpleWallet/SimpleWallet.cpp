         ...
         Config &config)
{
  ...
  do
  {
    std::cout << InformationMsg("BLOC v" + std::string(PROJECT_VERSION) + " Simplewallet") << std::endl;
  ...
  {
    std::cout << WarningMsg("It looks like BLOCd isn't open!")
              << std::endl
              << std::endl
              << WarningMsg("Ensure BLOCd is open and has finished "
                            "initializing.")
              << std::endl
              << WarningMsg("If it's still not working, try restarting "
                            "BLOCd. The daemon sometimes gets stuck.")
              << std::endl
              << WarningMsg("Alternatively, perhaps BLOCd can't "
              ...
              bool &alreadyShuttingDown)
{
  ...
  {
    std::cout << "Hold on, we're already shutting down!"
              << std::endl;
    return false;
  }
  else
...
void inputLoop(std::shared_ptr<WalletInfo> &walletInfo, CryptoNote::INode &node)
{
  while (true)
  {
    ...
    /* Split into args to support legacy transfer command, for example
           transfer 5 abLocxyz... 100, sends 100 BLOC to abLocxyz... with a mixin
           of 5 */
