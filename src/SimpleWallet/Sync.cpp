                           ...
                           CryptoNote::INode &node)
{
    ...
    /* Couldn't get timestamp, maybe old node or BLOCd closed */
    if (blockTime != "")
    {
                ...
                std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    {
        std::cout << "Your BLOCd isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting BLOCd.\n"
                    "If this persists, visit "
                    "https://bloc.money/#contact for support.";
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[BLOC " + shortName + "]: ");
}
