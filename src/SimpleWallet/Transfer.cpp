    ...
    while (true)
    {
      ...
      if (neededBalance < wallet.getActualBalance())
      {
        ...
        std::cout << SuccessMsg("Transaction has been sent!")
                  << std::endl
                  << SuccessMsg("Hash: " + Common::podToHex(sentTx.hash))
                  << std::endl
                  << std::endl;
             ...
             CryptoNote::TransactionParameters p)
{
  std::cout << "Wallet optimization failed, transactions are still too "
            ...
            << std::endl
            << "This may take a long time as portions of your "
            << "balance get locked whilst sending a transaction."
            << std::endl
            << "It may also slightly raise the fee you have "
  ...
  for (int numTxMultiplier = 2;; numTxMultiplier++)
  {
    ...
    /* We can't just evenly divide a transaction up to be < 115k bytes by
           ...
           transactions. */
    int numTransactions = int(numTxMultiplier *
                              (std::ceil(double(txSize) / double(maxSize))));

    /* Split the requested fee over each transaction, i.e. if a fee of 200
           BLOC was requested and we split it into 4 transactions each one will
           have a fee of 5 BLOC. If the fee per transaction is less than the
    ...
    catch (const std::system_error &e)
    {
      ...
      case NodeErrors::CryptoNote::error::CONNECT_ERROR:
      {
        std::cout << WarningMsg("Couldn't connect to the network "
                  ...
                  << std::endl
                  << "Ensure BLOCd or the remote node you "
...
Maybe<uint64_t> getTransferAmount()
{
  while (true)
  {
    ...
    std::cout << std::endl
              << InformationMsg("How much BLOC do you want to send?: ");
...
bool parseFee(std::string feeString)
{
  ...
  {
    std::cout << WarningMsg("Fee must be at least 0.0001 BLOC!") << std::endl;
    return false;
  }
...
bool parseAddress(std::string address)
{
  ...
  /* We can't get the actual prefix if the address is invalid for other
       ...
       with BLOC, aslong as the prefix is the BLOC prefix. This keeps it
       working on testnets with different prefixes. */
  else if (expectedPrefix == 0xee06ac8 && address.substr(0, 5) != "abLoc")
  {
    std::cout << WarningMsg("Invalid address! It should start with abLOC!")
              << std::endl
              << std::endl;
    return false;
  }
...
bool parseMixin(std::string mixinString)
{
  try
  {
    ...
    uint16_t minMixin = std::max(CryptoNote::parameters ::MINIMUM_MIXIN_NO_DUST,
...
bool parseAmount(std::string amountString)
{
  ...
  if (!parseAmount(amountString, amount))
  {
    std::cout << WarningMsg("Failed to parse amount! Ensure you entered "
                            "the value correctly.")
              << std::endl
              << "Please note, the minimum you can send is 0.0001 BLOC,"
              << std::endl
